...
(function(){
	...
	var validqte == function(query){ //creating a function named validate with an parameter of query
		...
		for(var i=0, j=db.length; i<j; i++) { //for loop, to go through every item in database
			...
			for (var ii = 0, jj = queryArray.length; ii < jj; ii++) { //for loop
				...
				if (compare !== -1) {  // if loop, if search query matches a database item
					results.push(db[i]); //add the database item to the array with stored match reesults
				};   //end if loop
			};
		};
